...
test("issues a 15 minute token bound to the anonymous session and spot without coordinates", () => {
  const result = issuePresenceToken({ userId: "user-1", spotId: "spot-1" }, secret, now, "nonce-1");
...
test("rejects tampering, expiry and binding mismatches", () => {
  const { token } = issuePresenceToken({ userId: "user-1", spotId: "spot-1" }, secret, now, "nonce-1");
