-- Owners must see every accepted submission in their own field, including a
-- melon held for safety review. Held melons remain private and never earn the
-- daily true-seed reward until the moderation policy explicitly says so.
...
as $$
  with target as (
    ...
    where m.author_id = p_profile_id
      and (
        public.effective_melon_status(m.status, m.matures_at, m.completed_reads) in ('incubating', 'mature')
        or (p_include_private and m.status = 'held')
      )
      and (
