-- Make like/squat writes server-authoritative and bind comment presence to the
-- melon itself. Nearby-area validation happens in the API with a one-time
-- location proof and the existing HMAC cell; no coordinates are stored here.

drop function if exists public.set_melon_reaction(uuid, public.reaction_type, boolean);
drop function if exists public.set_melon_squat(uuid, boolean);
...
create or replace function public.get_melon_presence_target(p_melon_id uuid, p_actor_id uuid)
returns jsonb
language sql
stable
security definer
...
drop function if exists public.add_melon_comment(uuid, uuid, uuid, text);
...
revoke all on function public.get_melon_presence_target(uuid, uuid) from public, anon, authenticated;
...
grant execute on function public.get_melon_presence_target(uuid, uuid) to service_role;
